...
class MyApp(QWidget):
    def __init__(self):
        ...
        self.function_input_mode = False 
        self.initUI()
    ...
    def number_btn_clicked(self):
        ...
        if self.function_input_mode:
            self.current_num += number
            self.expr = f"{self.pending_function}({self.current_num}"
    ...
    def function_btn_clicked(self):
        ...
        try:
            ...
            # ✅ 삼각함수들 - 함수명 + "(" 표시
                ...
                self.start_function_input(function)
            ...
            # ✅ 즉시 계산되는 함수들
                ...
                result = self.calculator.square(self.current_num)
                ...
                result = self.calculator.cube(self.current_num)
                ...
                self.expr += str(self.calculator.pi())
                self.current_num = self.calculator.pi()
                self.display.setText(self.expr)
    ...
    def start_function_input(self, func_name):
        self.pending_function = func_name
        self.function_input_mode = True
        self.expr = f"{func_name}("
        self.display.setText(self.expr)
        self.has_decimal_point = False
        self.current_num = ""
    ...
    def equals_btn_clicked(self):
        ...
        try:
            if self.function_input_mode and self.pending_function:
                if self.current_num == "":
                    raise ValueError("함수에 인수가 필요합니다")

                func_map = {
                    'sin': self.calculator.sin,
                    'cos': self.calculator.cos, 
                    'tan': self.calculator.tan,
                    'sinh': self.calculator.sinh,
                    'cosh': self.calculator.cosh,
                    'tanh': self.calculator.tanh
                }

                result = func_map[self.pending_function](self.current_num)
    ...
    def initUI(self):
        ...
        self.btn_e_power_x = self.create_button('eˣ', 1, 4, 'function')
        self.btn_10_power_x = self.create_button('10ˣ', 1, 5, 'function') 
...
class Calculator:
    ...
    def sin(self, value):
        try:
            num = self.validate_result(value)
            result = math.sin(math.radians(num))
            return self.validate_result(result)
        except Exception as e:
            raise ValueError(f"사인 계산 오류: {e}")
    ...
    def cos(self, value):
        try:
            ...
            raise ValueError(f"코사인 계산 오류: {e}")
    ...
    def tan(self, value):
        try:
            ...
            if abs(math.cos(math.radians(num))) < 1e-10:
                raise ValueError("탄젠트가 정의되지 않습니다")
            
            result = math.tan(math.radians(num))
            return self.validate_result(result)
        except Exception as e:
            raise ValueError(f"탄젠트 계산 오류: {e}")
    ...
    def sinh(self, value):
        try:
            ...
            raise ValueError(f"쌍곡선 사인 계산 오류: {e}")
    ...
    def cosh(self, value):
        try:
            ...
            raise ValueError(f"쌍곡선 코사인 계산 오류: {e}")
    ...
    def tanh(self, value):
        try:
            ...
            raise ValueError(f"쌍곡선 탄젠트 계산 오류: {e}")
    ...
    def cube(self, value):
        try:
            ...
            raise ValueError(f"세제곱 계산 오류: {e}")
    
    def pi(self):
        return self.validate_result(math.pi)
