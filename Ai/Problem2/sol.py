...
def print_gender(origin_df):
    ...
    gender_stats = origin_df[(origin_df['성별'].isin(['남자', '여자'])) & (origin_df['연령별'] == '합계')][['시점', '성별', '연령별','일반가구원']].copy()
...
def print_age(origin_df):
    ...
    age_stats = filtered_df.groupby('연령별', as_index=False)['일반가구원'].sum()
...
def print_age_gender(origin_df):
    ...
    age_gender_stats = filtered_df.groupby(['연령별', '성별'])['일반가구원'].sum().reset_index()
    pivot_stats = age_gender_stats.pivot(index='연령별', columns='성별', values='일반가구원').reset_index()
    print(pivot_stats.to_string(index=False))
    return pivot_stats

def draw_graph(graph_data):
    plt.figure(figsize=(12, 6))
    plt.plot(graph_data['연령별'], graph_data['남자'], marker='o', label='남자')
    plt.plot(graph_data['연령별'], graph_data['여자'], marker='o', label='여자')
    plt.xlabel('연령별')
    plt.ylabel('일반가구원')
